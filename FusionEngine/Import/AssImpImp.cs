    ...
    public class AssImpImport : Importer
    {
        ...
        public override Node3D LoadAnimNode ( string path )
        {
            ...
            if ( sc > 0 )
            {
                ...
                m1.TSpec = new Texture.Texture2D ( IPath + "/" + tex2.FilePath, Texture.LoadMethod.Single, false );
            }
            ...
            if ( mat.GetMaterialTextureCount ( TextureType.Diffuse ) > 0 )
            {
                ...
                if ( t1.FilePath != null )
                {
                    try
                    {
                        ...
                        m1.TCol = new Texture.Texture2D ( IPath + "/" + t1.FilePath.Replace ( ".dds", ".png" ), Texture.LoadMethod.Single, false );
        ...
        public override Node3D LoadNode ( string path )
        {
            ...
            foreach ( Mesh m in s.Meshes )
            {
                ...
                {
                    TextureSlot tex2 = mat.GetMaterialTextures(TextureType.Unknown)[0];
                    vm.TSpec = new Texture.Texture2D ( IPath + "/" + tex2.FilePath, Texture.LoadMethod.Single, false );
                }
                ...
                if ( mat.GetMaterialTextureCount ( TextureType.Diffuse ) > 0 )
                {
                    ...
                    if ( t1.FilePath != null )
                    {
                        ...
                        // Console.Write("t1:" + t1.FilePath);
                        vm.TCol = new Texture.Texture2D ( IPath + "/" + t1.FilePath.Replace ( ".dds", ".png" ), Texture.LoadMethod.Single, false );
                        if ( File.Exists ( IPath + "/" + "norm_" + t1.FilePath ) )
                        {
                            vm.TNorm = new Texture.Texture2D ( IPath + "/" + "norm_" + t1.FilePath, Texture.LoadMethod.Single, false );
                        }
                    }
                }
